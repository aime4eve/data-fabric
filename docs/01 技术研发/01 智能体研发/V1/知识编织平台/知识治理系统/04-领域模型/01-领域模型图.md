...
## 目录与权限子域 (Directory & Access Control)
...
classDiagram
    class DirectoryAggregate {
        ...
        +boolean hasChildren
        +Version version
        +List~DirectoryAggregate~ children
        +rename(DirectoryName)
        +moveTo(DirectoryId)
        +sort(int)
        +addChild(DirectoryAggregate)
        +removeChild(DirectoryId)
    }
    ...
    class PermissionConfig {
        ...
        +breakInheritance()
        +setRules(List~PermissionRule~)
        +effectivePermissions(DirectoryAggregate): List~PermissionRule~
    }

    class PermissionRule {
        <<ValueObject>>
        +SubjectType subjectType  // user|group|role
        +SubjectId subjectId
        +Set~Action~ actions      // view|edit|manage
    }
    ...
    class Template {
        ...
        +DefaultPermissionsJson defaultPermissions
        +preview(): StructureJson
    }

    class DirectoryDomainService {
        <<DomainService>>
        +applyTemplate(Template, KnowledgeBaseId): InitResult
        +recalculatePath(DirectoryAggregate): void
    }

    class AuditEvent {
        <<Entity>>
        +AuditId id
        +String type // create|rename|move|delete|permission
        +Operator operator
        +DateTime timestamp
        +String before
        +String after
    ...
    DirectoryAggregate *-- PermissionConfig
    DirectoryAggregate o-- DirectoryAggregate : children
    DirectoryDomainService ..> Template
    PermissionConfig o-- PermissionRule
    DirectoryAggregate o.. AuditEvent : emits

    class DirectoryId{ <<ValueObject>> +String value }
    class KnowledgeBaseId{ <<ValueObject>> +String value }
    class DirectoryName{ <<ValueObject>> +String value +validate() }
    class Slug{ <<ValueObject>> +String value }
    class Path{ <<ValueObject>> +String value }
    class Version{ <<ValueObject>> +String value }

```

**不变量与约束**
- 同一父级下 `DirectoryName` 唯一；`Slug` 由名称规范化生成。
- `depth ≤ 6`（可配置）；每层子节点数 `≤ 200`（可配置）。
- 移动目录时需重算 `Path`，并级联更新子路径（事务或异步修复）。
- 权限默认继承父级；支持断开继承与局部覆盖，计算有效权限按父链合并。
