...
## 目录与权限聚合 (DirectoryAggregate & PermissionConfig)

### 聚合根: DirectoryAggregate
```typescript
class DirectoryAggregate {
    private readonly id: DirectoryId;
    private readonly knowledgeBaseId: KnowledgeBaseId;
    private parentId?: DirectoryId;
    ...
    public rename(newName: DirectoryName): void {
        this.validateName(newName);
        this.name = newName;
        this.slug = Slug.fromName(newName);
        this.updatedAt = DateTime.now();
        this.addDomainEvent(new DirectoryRenamed(this.id, newName));
    }

    public moveTo(newParentId: DirectoryId): void {
        this.validateMove(newParentId);
        this.parentId = newParentId;
        this.recalculatePath();
        this.updatedAt = DateTime.now();
        this.addDomainEvent(new DirectoryMoved(this.id, newParentId));
    }

    public sort(newOrder: number): void {
        this.order = newOrder;
        this.updatedAt = DateTime.now();
    }

    private recalculatePath(): void {
        this.path = Path.recalculate(this.parentId, this.slug);
    }

    private validateName(name: DirectoryName): void {
        if (name.isEmpty() || name.length() > 64) {
            throw new DomainException("目录名不合法");
        }
    }

    private validateMove(newParentId: DirectoryId): void {
        if (this.depth >= 6) {
            throw new DomainException("超过最大层级");
        }
    }
}
```

### 聚合实体: PermissionConfig & PermissionRule
```typescript
class PermissionConfig {
    private directoryId: DirectoryId;
    private inherit: boolean = true;
    private rules: Set<PermissionRule> = new Set();
    private version: Version;
    ...
    public setRules(rules: Set<PermissionRule>): void {
        this.rules = rules;
    }

    public effectivePermissions(
        parentConfig: PermissionConfig | null
    ): Set<PermissionRule> {
        if (this.inherit && parentConfig != null) {
            return PermissionMerger.merge(parentConfig.rules, this.rules);
        }
        return this.rules;
    }
}
...
class PermissionRule {
    private subjectType: SubjectType; // user|group|role
    private subjectId: SubjectId;
    private actions: Set<Action>;     // view|edit|manage
}
```
...
class DirectoryDomainService {
    public applyTemplate(
        ...
        // 幂等应用：跳过已存在节点，仅补缺失
        // 返回 createdNodes 与 skippedNodes
...
class AuditService {
    public record(event: AuditEvent): void {
        // 记录 create|rename|move|delete|permission
    }
}
```
...
interface DirectoryRepository {
    save(dir: DirectoryAggregate): Promise<void>
    findById(id: DirectoryId): Promise<DirectoryAggregate | null>
    findChildren(parentId: DirectoryId): Promise<DirectoryAggregate[]>
}

interface PermissionRepository {
    loadConfig(directoryId: DirectoryId): Promise<PermissionConfig>
...
### 业务不变量与边界
- 同父级目录名唯一；`depth ≤ 6`，子数限制可配置。
- 移动需重算路径并级联子节点路径；事务或异步修复。
- 权限默认继承；支持断开继承与覆盖；有效权限按父链合并。
- 并发控制：`version` 乐观锁；接口更新需携带 `If-Match`。